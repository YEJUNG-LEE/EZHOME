$(document).ready(function(){
    ...
    var trade_inputLeft = document.getElementById("trade-input-left");
    var trade_inputRight = document.getElementById("trade-input-right");
    var trade_inputMax = trade_inputRight.getAttribute('max');


    var trade_thumbLeft = document.querySelector(".tradeSlider > .thumb.left");
    var trade_thumbRight = document.querySelector(".tradeSlider > .thumb.right");
    var trade_range = document.querySelector(".tradeSlider > .range");
    var trade_text_div = document.querySelector(".ReEs-navi-normal-trade-select-slider-txt");

    if(trade_inputRight.value == trade_inputMax){
    trade_text_div.innerHTML = trade_inputLeft.value + " - 최대";
    }else{
    trade_text_div.innerHTML = trade_inputLeft.value + " - " + trade_inputRight.value + "만원";
    }

    var trade_setLeftValue = () => {
    var _this = trade_inputLeft;
    var [min, max] = [parseInt(_this.min), parseInt(_this.max)];

    // 교차되지 않게, 1을 빼준 건 완전히 겹치기보다는 어느 정도 간격을 남겨두기 위해.
    _this.value = Math.min(parseInt(_this.value), parseInt(trade_inputRight.value) - 1);

    // input, thumb 같이 움직이도록
    var percent = ((_this.value - min) / (max - min)) * 100;
    trade_thumbLeft.style.left = percent + "%";
    trade_range.style.left = percent + "%";
    if(trade_inputRight.value == trade_inputMax){
        trade_text_div.innerHTML = trade_inputLeft.value + " - 최대";
    }else{
        trade_text_div.innerHTML = trade_inputLeft.value + " - " + trade_inputRight.value + "만원";
    ...
    var trade_setRightValue = () => {
    var _this = trade_inputRight;
    var [min, max] = [parseInt(_this.min), parseInt(_this.max)];

    // 교차되지 않게, 1을 더해준 건 완전히 겹치기보다는 어느 정도 간격을 남겨두기 위해.
    _this.value = Math.max(parseInt(_this.value), parseInt(trade_inputLeft.value) + 1);

    // input, thumb 같이 움직이도록
    var percent = ((_this.value - min) / (max - min)) * 100;
    trade_thumbRight.style.right = 100 - percent + "%";
    trade_range.style.right = 100 - percent + "%";
    if(trade_inputRight.value == trade_inputMax){
        trade_text_div.innerHTML = trade_inputLeft.value + " - 최대";
    }else{
        trade_text_div.innerHTML = trade_inputLeft.value + " - " + trade_inputRight.value + "만원";
    ...
    trade_inputLeft.addEventListener("input", trade_setLeftValue);
    trade_inputRight.addEventListener("input", trade_setRightValue);

    floor_setLeftValue();
    floor_setRightValue();
    extent_setLeftValue();
    extent_setRightValue();
    month_setLeftValue();
    month_setRightValue();
    manage_setLeftValue();
    manage_setRightValue();
    deposit_setLeftValue();
    deposit_setRightValue();
    trade_setLeftValue();
    trade_setRightValue();
})
